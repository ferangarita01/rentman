...
async function fetchSecrets() {
    // Check if running in Vercel or CI
    if (process.env.VERCEL || process.env.CI) {
        console.log('🚀 Running in Vercel/CI environment.');
        console.log('⚠️ Skipping Google Secret Manager fetch. Ensure environment variables are set in Vercel Project Settings.');

        // Create an empty .env.local to satisfy the build requirement if necessary, 
        // essentially expecting Vite to pick up system env vars.
        const envPath = path.resolve(process.cwd(), '.env.local');
        fs.writeFileSync(envPath, '# Environment variables provided by Vercel\n');
        return;
    }

    console.log('🔐 Fetching secrets from Google Secret Manager (via gcloud CLI)...');
    ...
    for (const [secretName, envVarName] of Object.entries(SECRET_MAPPING)) {
        try {
            ...
            if (payload) {
                ...
                console.log(`  ✅ Loaded ${secretName} -> ${envVarName}`);
            } else {
                console.warn(`  ⚠️ Empty payload for ${secretName}`);
            }
        } catch (error) {
            console.error(`  ❌ Failed to fetch ${secretName}:`, (error as Error).message);
...
fetchSecrets().catch(err => {
    console.error('❌ Fatal error fetching secrets:', err);
    process.exit(1);
});
